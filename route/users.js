...
async function updateWebHook(json){
    ...
    let res = await KycModel.findOneAndUpdate ({customercode:json.data.customer_code,'level2.email':json.data.email},{'level2.[0].event_status':json.event},null,async(err)=>{
        if(err){
            // console.log('Error',err)
            return [false,err]
        }
